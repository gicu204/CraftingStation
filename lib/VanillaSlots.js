...
var defaultFunctions = {
    SlotToSlot: function(container, eventData, connectedClient) {
        ...
        container.sendChanges();
            ...
            // var _slot1 = parseSlot(slot1.id);
            // var _slot2 = parseSlot(slot2.id);
            
            // let tile = container.getParent();

            // let chest = StorageInterface.getNeighbourStorage(
            //     this.blockSource, {x: tile.x, y: tile.y, z: tile.z}, _slot1.side
            // );
            // //if (!chest) return;
            // chest.setSlot(_slot1.slot, slot1.id, slot1.count, slot1.data, slot1.extra);

            // let chest2 = StorageInterface.getNeighbourStorage(
            //     this.blockSource, {x: tile.x, y: tile.y, z: tile.z}, _slot2.side
            // );
            // //if (!chest) return;
                        ...
                        if(currentSlotType){
                            for(var i in slots){
                                ...
                                chestData.container.sendEvent("SlotToSlot", {slot1: i, slot2: slot_id, value: value});
                                    //chestData.container.sendEvent("sTs",{slot1: i, slot2: slot_id});
...
// LocalTick handled in ChestTransferLogic.js (dev/blocks/) — do not duplicate
var VanillaSlots_ = {
...
EXPORT("VanillaSlots_station", VanillaSlots_);